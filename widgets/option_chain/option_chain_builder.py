...
def build_chain(
    ...
    Records use the canonical schema: strike in rupees, ``instrument_type`` is
    ``"CE"``/``"PE"``, ``expiry`` is ISO ``YYYY-MM-DD``.
    """
    ...
    for _, _, record in InstrumentMaster._index:
        ...
        if (
            ...
            or record.get("exchange") != exchange
            or inst_type not in _OPT_TYPES
        ):
            continue

        try:
            actual_strike = float(record.get("strike", -1))
...
def get_expiries(underlying_name: str, exchange: str = "NFO") -> list[str]:
    ...
    for _, _, record in InstrumentMaster._index:
        if (
            ...
            and record.get("instrument_type") in _OPT_TYPES
        ):
